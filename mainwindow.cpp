...
void MainWindow::extractText()
{
    ...
    char *old_ctype = _strdup(setlocale(LC_ALL, NULL));
    setlocale(LC_ALL, "C");
    if (tesseractAPI == nullptr) {
        tesseractAPI = new tesseract::TessBaseAPI();
//        tesseractAPI.SetVariable("user_defined_dpi", "300");
        // Initialize tesseract-ocr with English, with specifying tessdata path
//        qDebug()<<"TESSDATA_PREFIX"<<"C:/tesseract/bin";
//        if (tesseractAPI->Init(TESSDATA_PREFIX, "eng"))
        if (tesseractAPI->Init("C:/tesseract/tessdata", "eng"))
        {
...
//        editor->setPlainText("");
        ...
        for(cv::Rect &rect : areas) {
            ...
            str = /*str +*/ QString(QLatin1String(outText));
//            editor->setPlainText(str);
            editor->setPlainText(editor->toPlainText() + str);
//            editor->setPlainText(editor->toPlainText() + outText);
//            delete [] outText;  //删除程序会闪退
        }
    } else {
        char *outText = tesseractAPI->GetUTF8Text();
        qDebug()<<"outText"<<outText;
        QString  str = QString(QLatin1String(outText));
        editor->setPlainText(str);
//        delete [] outText;  //删除程序会闪退
    }
